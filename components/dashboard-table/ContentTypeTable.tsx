...
const ContentTypeTable = () => {
      ...
      const [currentPage, setCurrentPage] = useState(0);
            ...
            const {data: contentTypeData, loading, totalData} = useFetch(
                `/api/content-type?${globalFilter && `type=${globalFilter}`}&page=${currentPage}&limit=${perPage}`,
        ...
        try {
            ...
            const res = await fetch(`/api/content-type${id ? `/${id}` : ''}`, {
            ...
            if (res.ok) {
                ...
                toast.success("Create content type successfully.");
            } else {
        ...
        <div>
            ...
            <Table>
                ...
                <TableBody>
                    ...
                    {/* Empty State */}
                    {!loading && contentTypeData?.length === 0 && (
                        <TableRow>
                    ...
                    {/* Data State */}
                    {!loading && contentTypeData?.map((type: ContentTypeType) => (
                        <TableRow key={type?._id}>
                            <TableCell className="font-medium">{type?.name}</TableCell>
                            <TableCell>{type?.description || "No description provided"}</TableCell>
                            <TableCell>
            ...
            <Dialog open={isEditModalOpen} onOpenChange={handleCloseModal}>
                <DialogContent className="max-w-300">
                    <DialogHeader>
                        <DialogTitle>Create/Update content type</DialogTitle>
                        <DialogDescription>
                            Create or Update a new content type to add to your list.
                        </DialogDescription>
                    </DialogHeader>
