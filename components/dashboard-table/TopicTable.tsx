...
import { DropdownType, TopicType } from '@/types';
...
const TopicTable = () => {
     const [currentPage, setCurrentPage] = useState(0);
        ...
        const {data: topicData, loading, totalData} = useFetch(
            `/api/topic?${globalFilter && `topic=${globalFilter}`}&page=${currentPage}&limit=${perPage}`,
    ...
    const handleSubmit = async (id ?: string | null) => {
        ...
        const payload = {
            subject: formData?.subject?._id || '',
            name: formData?.name || '',
            description: formData?.description || ''
        }

         if (!payload.subject) {
            error.subject = 'Subject name is required'
        }
    ...
    return (
            ...
            <Table>
                ...
                <TableBody>
                    ...
                    {/* Data State */}
                    {!loading && topicData?.map((topic: TopicType) => (
                        <TableRow key={topic?._id}>
                            <TableCell className="font-medium">{topic?.name}</TableCell>
                            <TableCell className="font-medium">{topic?.subject?.name}</TableCell>
                            <TableCell>{topic?.description || "No description provided"}</TableCell>
                            <TableCell>
            ...
            <Dialog open={isEditModalOpen} onOpenChange={handleCloseModal}>
                <DialogContent className="max-w-300">
                    <DialogHeader>
                        <DialogTitle>Create/Update Topic</DialogTitle>
                        <DialogDescription>
                            Create or Update a new Topic to add to your list.
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="space-y-2">
                            ...
                            <SingleSelect data={subjectData} dataLoading={subjectLoading} value={formData?.subject} setValue={(v) => { setFormData(prev => ({...prev, subject: v}))}} />
                            {errors?.subject && (
