...
function PaginationPageLink({
    ...
    return (
        <PaginationItem key={pageNum}>
            {!isActive ? (
                <Button variant="default" disabled className="font-bold disabled:opacity-100">{pageNum + 1}</Button>
            ) : (
                <PaginationLink
...
export default function CustomPagination({
    ...
    return (
        <Pagination>
            <PaginationContent>
                {showPreviousNext && currentPage > 0 ? (
                    <PaginationItem>
                        <PaginationPrevious
                ...
                {generatePaginationLinks(currentPage, totalPages - 1, onPageChange)}
                {showPreviousNext && currentPage < (totalPages - 1) ? (
                    <PaginationItem>
                        <PaginationNext
                            onClick={() => onPageChange(currentPage + 1)}
                            isActive={true}
